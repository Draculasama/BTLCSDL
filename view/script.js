import { findSanPhamByName } from './api.js';

const API_BASE_URL = "https://btlcsdl.onrender.com";

const productSearch = document.getElementById('productSearch');
if (productSearch) productSearch.addEventListener('keypress', (e) => { 
    if (e.key === 'Enter') { 
        searchProducts(); 
    } 
});
...
async function searchProducts() {
    ...
    if (!searchTerm) {
        // Nếu không nhập gì thì có thể load lại toàn bộ sản phẩm
        loadProducts();
        return;
    }
    try {
        const products = await findSanPhamByName(searchTerm);
        renderProducts(products); // Hàm này sẽ hiển thị danh sách sản phẩm ra giao diện
    } catch (error) {
